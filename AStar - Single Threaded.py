...
class AStar_Solver:
    ...
    def Solve(self):
        ...
        if startState != None:
            ...
            end_time = time.time()
            self.time_taken = round(float((end_time - start_time) * 1000), 0)
            self.paths_considered = count
            return self.path
        else:
...
class Worker(threading.Thread):
    def __init__(self, PriorityQueue, visitedQueue, allowed_states):
...
def String_Solver_Example():
    ...
    print("Paths Considered: " + str(a.paths_considered))
